...
if (!is.element(test,x)) stop(paste("test should be one of ",x,sep=""))
threshold = 0.05
n = ncol(data$expression)
m = nrow(data$expression)
if (length(pvalues)==0) pvalues = raw.pvalues(data,x,test)$pval
SelectH0=(1:m)[pvalues>threshold]
y = t(data$expression)
xtest = which(test==x)
covar = data$covariates[,c(x[-xtest],test)]
X0 = matrix(rep(1,n),ncol=1)
if (length(x)>1) X0 = model.matrix(y~.,data=data.frame(y=rep(1,n),covar[,-ncol(covar)]))
P0 = diag(n)-X0%*%solve(t(X0)%*%X0)%*%t(X0)
X1 = model.matrix(y~.,data=data.frame(y=rep(1,n),covar))
